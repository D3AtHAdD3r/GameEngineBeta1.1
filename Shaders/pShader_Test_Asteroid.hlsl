...
Texture2D Texture[125] : register(t0);
...
{
    float4 texColor;
    if (TerrainSize.y < 100)
    {
        texColor = Texture[122].Sample(TextureSampler, float2(input.texcoord.x, 1.0f - input.texcoord.y));
    }
    else
    {
        texColor = Texture[0].Sample(TextureSampler, float2(input.texcoord.x, 1.0f - input.texcoord.y));
    }
